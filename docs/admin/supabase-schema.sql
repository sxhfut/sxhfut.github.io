...
create table if not exists public.content_tasks (
  id uuid primary key default gen_random_uuid(),
  task_type text not null default 'public_update',
  title text not null,
  summary text,
  source_url text,
  target_url text,
...
create table if not exists public.frontier_curations (
  id uuid primary key default gen_random_uuid(),
  title text not null,
  source_url text not null,
  track text not null default 'AI + Psychology',
  summary text,
  takeaway_zh text,
  takeaway_en text,
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint frontier_curations_source_url_unique unique (source_url)
);
...
drop trigger if exists content_tasks_touch_updated_at on public.content_tasks;
create trigger content_tasks_touch_updated_at
  before update on public.content_tasks
  for each row execute function public.touch_updated_at();

drop trigger if exists frontier_curations_touch_updated_at on public.frontier_curations;
create trigger frontier_curations_touch_updated_at
  before update on public.frontier_curations
  for each row execute function public.touch_updated_at();

drop trigger if exists lab_news_audit_log on public.lab_news;
create trigger lab_news_audit_log
  after insert or update or delete on public.lab_news
  for each row execute function public.write_content_audit_log();

drop trigger if exists lab_outputs_audit_log on public.lab_outputs;
create trigger lab_outputs_audit_log
  after insert or update or delete on public.lab_outputs
  for each row execute function public.write_content_audit_log();

drop trigger if exists partner_requests_audit_log on public.partner_requests;
create trigger partner_requests_audit_log
  after insert or update or delete on public.partner_requests
  for each row execute function public.write_content_audit_log();

drop trigger if exists content_tasks_audit_log on public.content_tasks;
create trigger content_tasks_audit_log
  after insert or update or delete on public.content_tasks
  for each row execute function public.write_content_audit_log();

drop trigger if exists frontier_curations_audit_log on public.frontier_curations;
create trigger frontier_curations_audit_log
  after insert or update or delete on public.frontier_curations
  for each row execute function public.write_content_audit_log();
...
drop policy if exists "content_tasks_select_scope" on public.content_tasks;
create policy "content_tasks_select_scope"
  on public.content_tasks for select
  using (submitted_by = auth.uid() or assignee_id = auth.uid() or public.is_console_admin());

drop policy if exists "content_tasks_insert_authenticated" on public.content_tasks;
create policy "content_tasks_insert_authenticated"
  on public.content_tasks for insert
  with check (public.current_console_role() is not null and submitted_by = auth.uid());

drop policy if exists "content_tasks_update_scope" on public.content_tasks;
create policy "content_tasks_update_scope"
  on public.content_tasks for update
  using (submitted_by = auth.uid() or assignee_id = auth.uid() or public.is_console_admin())
  with check (((submitted_by = auth.uid() or assignee_id = auth.uid()) and public.current_console_role() is not null) or public.is_console_admin());

drop policy if exists "frontier_curations_select_scope" on public.frontier_curations;
create policy "frontier_curations_select_scope"
  on public.frontier_curations for select
  using (submitted_by = auth.uid() or public.is_console_admin());

drop policy if exists "frontier_curations_insert_authenticated" on public.frontier_curations;
create policy "frontier_curations_insert_authenticated"
  on public.frontier_curations for insert
  with check (public.current_console_role() is not null and submitted_by = auth.uid());

drop policy if exists "frontier_curations_update_admin" on public.frontier_curations;
create policy "frontier_curations_update_admin"
  on public.frontier_curations for update
  using (public.is_console_admin())
  with check (public.is_console_admin());

drop policy if exists "content_audit_logs_select_scope" on public.content_audit_logs;
create policy "content_audit_logs_select_scope"
